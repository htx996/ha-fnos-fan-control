...
class DashboardVerifications(VerifyCase):
    ...
    def verify_dashboard_yaml_has_required_dynamic_sections(self):
        ...
        self.assertIn('background: "#0c1114"', view)
        self.assertIn("background: transparent", view)
        self.assertIn("- border: 1px solid rgba(255, 255, 255, 0.055)", templates)
    ...
    def verify_dashboard_uses_graphical_storage_disk_and_fan_views(self):
        ...
        self.assertIn("font-family: -apple-system", view)
        self.assertIn("backdrop-filter: blur(18px)", view)
        self.assertIn("prefers-reduced-motion: reduce", templates)
        self.assertIn("ha-card:active", templates)
        self.assertIn("fn_nas_dashboard_role: power", view)
        ...
        self.assertEqual(view.count("fn_nas_dashboard_category: control"), 3)
