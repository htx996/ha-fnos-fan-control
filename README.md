...
*   DXP4800 Pro 的 IT8613 硬件自动温控曲线尚未验证，因此只提供手动 PWM 调速，不再显示自动/全速模式下拉框；最低值限制为 PWM 80（约 31%），关闭 fan 实体也不会让散热风扇停转
*   从旧版本升级后如果风扇仍处于全速，请打开对应 fan 实体并设置为 40%-50%；集成会先切换到手动模式、写入 PWM，再读取 sysfs 确认写入结果
