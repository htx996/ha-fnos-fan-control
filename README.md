...
*   DXP4800 Pro 的 IT8613 硬件自动温控曲线尚未验证，因此模式下拉框只提供“手动、全速”，不提供会导致异常全速的“自动”；最低值限制为 PWM 80（约 31%），关闭 fan 实体也不会让散热风扇停转
*   从旧版本升级后如果风扇仍处于全速，请在模式下拉框选择“手动”，集成会自动恢复到 50%；也可以打开对应 fan 实体直接设置 40%-50%
*   手动调速使用单个 SSH 事务依次写入手动模式、目标 PWM、再次确认手动模式，等待 1 秒后同时读取两个 sysfs 文件；如果仍被系统改回全速，请复制实体属性中的“最近控制结果”用于定位外部控制器回写
