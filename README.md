...
*   `1.3.39` 在三张虚拟机品牌卡片中恢复重启按钮，卡片统一显示开机、关机、重启和当前运行状态
*   `1.3.40` 虚拟机卡片的开机、关机和重启按钮改为直接执行对应操作，不再打开实体详情页；操作完成后主动刷新虚拟机状态
*   `1.3.41` 虚拟机卡片支持长按开机、关机或重启按钮打开对应实体详情页，单击仍直接执行操作
*   `1.3.42` 虚拟机操作条改为单击左侧图标执行操作，单击文字或其余区域打开对应实体详情页，不再依赖长按
*   `1.3.43` 仪表盘重组为两条独立连续列，PC 和 Pad 的“设备详情”紧跟风扇区域，不再被另一列的图表高度推开；手机保持单列自适应
*   `1.3.44` 增加机型图片区的网格高度并适度放大 NAS 图片，避免图片覆盖下方运行时间和设备名称
*   `1.3.45` 取消 Homeassistant、iStoreOS 和 Windows 10 分组的外层卡片底色、边框与圆角，保留品牌图标、状态和操作按钮
*   `1.3.46` 所有 NAS、虚拟机和容器的开机、关机、重启、启动或停止操作在执行前显示确认弹窗，取消后不会发送控制命令
