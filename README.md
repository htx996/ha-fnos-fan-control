...
*   `1.3.33` 进一步压缩 NAS 图片、系统指标、存储卷、硬盘、风扇、控制项和温度趋势高度，在保留全部状态信息的前提下减少纵向留白
*   `1.3.34` 将系统电源和重启操作移至 NAS 机型图片右上角，改为仅显示图标的紧凑按钮，并移除按钮下方重复的状态文案
*   `1.3.36` 取消 `1.3.35` 的 Apple 风格外观，恢复统一无边框仪表盘；保留电源和重启按钮移出机型图片的防遮挡修复
*   `1.3.37` 将 NAS、虚拟机和 Docker 的双向电源开关拆分为独立的开机/关机或启动/停止按钮，并保留独立重启按钮；旧开关继续保留供已有自动化兼容，但不再显示在仪表盘
*   `1.3.38` 为 Home Assistant、iStoreOS 和 Windows 10 虚拟机增加独立品牌卡片，每张卡片集中显示官方图标、运行状态以及开机和关机按钮；重启实体继续保留，但不在紧凑卡片中显示
